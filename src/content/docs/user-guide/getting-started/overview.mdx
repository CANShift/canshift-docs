...
description: "Before you flash anything — what the dash does, what it talks to, and what you'll need."
sidebar:
  order: 1
---
...
<div class="cs-spec-sheet">
  ...
  <dl>
    <dt>Screen</dt><dd>Elecrow CrowPanel 2.8″ ESP32 (ILI9341 SPI + XPT2046 touch)</dd>
    <dt>CAN transceiver</dt><dd>NXP TJA1051T/3 breakout, 3.3 V tolerant</dd>
    <dt>Wiring</dt><dd>Dupont jumpers between screen ⇄ transceiver, OBD-II or direct splice to your ECU CAN H/L</dd>
    <dt>Cable</dt><dd>USB-C data cable (not power-only) for flashing</dd>
    <dt>Browser</dt><dd>Chromium-based (Chrome, Edge, Brave, Arc, Opera) — Firefox has no Web Serial</dd>
  </dl>
</div>
...
|---|---:|---:|
| [Wire up + first flash](/user-guide/install/first-flash/) | 4 min | ~5 min for a fresh board |
| [Pick an ECU profile, edit a page](/user-guide/configure/with-tuner/) | 3 min | ~10 min the first time |
| [Use the dash on the road](/user-guide/usage/navigating-pages/) | 2 min | 0 |

If something goes wrong, the boot logs are explicit — see [Boot diagnostics](/user-guide/install/boot-issues/) for the failure dictionary.

## Next

Read [First flash](/user-guide/install/first-flash/) — it walks the box-to-splash path in five steps, with a serial monitor on standby in case the boot logs need a second look.
