...
description: "Splash hangs, ErrorBar messages, USB silent — how to read the boot logs."
sidebar:
  order: 2
---
...
- Look for `[HEAP]` lines just before the failure — a `largest=` value below
  ...
  ordering is documented in [Boot sequence](/technical/architecture/boot-sequence/).
...
<Aside type="tip" title="The ErrorBar swipes up">
Swipe up from the red strip to expand the full error list. Dismiss individual
rows with the per-row × button.
</Aside>
