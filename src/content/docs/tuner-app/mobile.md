...
description: 'The phone app for in-car telemetry over BLE — early, and what it does today.'
---
...
The companion ([canshift-mobile](https://github.com/CANShift/canshift-mobile)) is a phone app for reading the dash in the car, where a laptop isn't practical. It connects over BLE — [pairing is here](/tuner-app/pairing/) — and mirrors the dash's telemetry on your phone.
...
It is not the configuration tool. Editing the dashboard, binding signals and burning a config is the [Tuner's](/tuner-app/tuner-tour/) job over USB; the phone is for watching, not building. That split is deliberate — BLE carries telemetry and a small command set, not the full contract ([Transports](/technical/architecture/transports/)).
